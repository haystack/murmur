			...
			for email in email_list:
				...
				if not member:
					...
					if WEBSITE == "murmur":
						...
						if email_user.count() == 1:
							mg,_ = MemberGroup.objects.get_or_create(member=email_user[0], group=group, active=False, hash=confirm_code)
							...
							mg,_ = MemberGroup.objects.get_or_create(group=group, member=new_user, active=False, hash=confirm_code)
