...
shared_patterns = [
    ...
    url(r'^create_new_group',
        'browser.views.create_group_view'),
    url(r'^create_group', 'browser.views.create_group'),
    url(r'^delete_group', 'browser.views.delete_group'),

    url(r'^groups/(?P<group_name>[\w-]+)/edit_group_info',
        'browser.views.edit_group_info_view'),
    url(r'^edit_group_info', 'browser.views.edit_group_info'),
    url(r'^group_info', 'browser.views.group_info'),
    url(r'^groups/(?P<group_name>[\w-]+)$',
        'browser.views.group_page'),

    url(r'^groups/(?P<group_name>[\w-]+)/add_members',
        'browser.views.add_members_view'),
    url(r'^add_members', 'browser.views.add_members'),
    url(r'^edit_members', 'browser.views.edit_members'),

    url(r'^unsubscribe_group',
        'browser.views.unsubscribe_group'),
    url(r'^subscribe_group', 'browser.views.subscribe_group'),

    url(r'^my_group_list', 'browser.views.my_group_list'),

    url(r'^edit_group_settings',
        'browser.views.edit_group_settings'),
    url(r'^group_settings', 'browser.views.get_group_settings'),
    url(r'^groups/(?P<group_name>[\w-]+)/edit_my_settings',
        'browser.views.my_group_settings_view'),

    url(r'^gmail_setup/',
        include('gmail_setup.urls', namespace="oauth2")),
    ...
    url(r'^attachment/(?P<hash_filename>[0-9A-Za-z_]+)',
        'browser.views.serve_attachment'),
    ...
    url(r'^subscribe/confirm/(?P<token>.+)$',
        'browser.views.subscribe_confirm'),
    ...
    url(r'^delete_mailbot_mode',
        'browser.views.delete_mailbot_mode'),
    url(r'^fetch_execution_log',
        'browser.views.fetch_execution_log'),
]
...
if WEBSITE == 'murmur':
    new_patterns = [
        ...
        url(r'^groups/(?P<group_name>[\w-]+)/add_list',
            'browser.views.add_list_view'),
        url(r'^groups/(?P<group_name>[\w-]+)/create_post',
            'browser.views.my_group_create_post_view'),
        ...
        url(r'^groups/(?P<group_name>[\w-]+)/add_donotsend',
            'browser.views.add_dissimulate_view'),
...
# squadbox-only patterns
    ...
    new_patterns = [
        ...
        url(r'^groups/(?P<group_name>[\w-]+)/add_whitelist',
            'browser.views.add_whitelist_view'),

        url(r'^unblacklist_unwhitelist',
            'browser.views.unblacklist_unwhitelist'),

        url(r'^blacklist_get', 'browser.views.blacklist_get'),
        url(r'^blacklist', 'browser.views.blacklist'),
        url(r'^groups/(?P<group_name>[\w-]+)/add_blacklist',
            'browser.views.add_blacklist_view'),

        url(r'^groups/(?P<group_name>[\w-]+)/rejected',
            'browser.views.rejected'),
        url(r'^rejected_thread$', 'browser.views.rejected_thread'),

        url(r'^moderate_user_for_thread_get',
            'browser.views.moderate_user_for_thread_get'),
    ]
